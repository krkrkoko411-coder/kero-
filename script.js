...
function finishStudy(minutes, subject) {
    ...
    if (typeof logActivity === 'function') {
        logActivity("study", subject, minutes);
    }
...
function notifyExam(exam){
    ...
    // Add to notification panel system
    if (typeof addKiroNotification === 'function') {
        const type = examDaysLeft(exam) === 0 ? 'warning' : 'exam';
        addKiroNotification(type, text);
    }
}
...
function finishPomoCycle(){
    ...
    if(pomoMode === "study"){
        ...
        // Log activity for streak tracking
        if (typeof logActivity === 'function') {
            logActivity("pomo", subject, data.pomodoro.study);
        }
...
function updateStreak(){
  ...
  if (hasToday) {
    ...
    if (days.includes(yesterdayStr)) {
      streak = 1; // Streak is still alive from yesterday
...
function renderNotificationPanel() {
  ...
  if (!notifications.length) {
    list.innerHTML = "<p>مفيش إشعارات جديدة.</p>";
    return;
  }

  list.innerHTML = notifications.map(n => `
    <div class="notice-row ${n.read ? '' : 'unread'}" data-id="${n.id}">
      ${esc(n.message)}
    </div>
  `).join("");

  // Add click handlers for marking as read
  list.querySelectorAll('.notice-row').forEach(row => {
    row.onclick = () => {
...
function setupNotificationPanel() {
  ...
  document.addEventListener('click', (e) => {
    if (panel.classList.contains('open') && !panel.contains(e.target) && !button.contains(e.target)) {
...
function setupSmartLayer(){
  ...
  renderDailyTasks(); updateSmartDashboard(); updateStreak(); checkAchievements(); showNotificationCenter(); setupNotificationPanel();
